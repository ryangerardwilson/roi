...
APP=""
EXPECTED_REPO_SLUG="ryangerardwilson/rgw_omarchy_installer"
REMOTE="origin"
REPO_SLUG=""
POLL_ATTEMPTS=60
...
require_standalone_repo() {
  local top_level
  top_level="$(git rev-parse --show-toplevel 2>/dev/null || true)"
  [[ -n "$top_level" ]] || die "No git repo found at $ROOT_DIR"
  [[ "$top_level" == "$ROOT_DIR" ]] \
    || die "Release from a standalone git repo rooted at $ROOT_DIR. Current git root is $top_level"
}
...
require_expected_remote() {
  local actual
  actual="$(remote_repo_slug)"
  [[ "$actual" == "$EXPECTED_REPO_SLUG" ]] \
    || die "origin must point to ${EXPECTED_REPO_SLUG}, found ${actual}"
}
...
release_is_published() {
  local version="$1"
  gh release view "v${version}" -R "$REPO_SLUG" >/dev/null 2>&1
}
